...
export const TrackerToggler = () => {
  ...
  return (
    <>
      {!isCopyTrackerOn ? (
        <Button className="relative" onClick={() => toggleCopyTrackerMode?.()}>
          Get started
          {prevTrackerStatus ?? (
            <svg
              className="fill-destructive pointer-events-none absolute top-0 right-0 size-4 translate-x-1/3 -translate-y-1/3"
              width="24"
              height="24"
              viewBox="0 0 24 24"
            >
